...
RSpec.describe 'posts/index.html.erb', type: :feature do
  describe 'index page' do
    let(:user) do
      User.create(name: 'Tom', photo: 'https://picsum.photos/id/64/200', bio: 'Teacher from Mexico')
    end
    let!(:post) { Post.create(user:, title: 'first ever post', text: 'this my post for the first time') }
    let!(:comment1) { Comment.create(user:, post:, text: 'Hi Tom') }
    let!(:comment2) { Comment.create(user:, post:, text: 'Hi Tom') }
